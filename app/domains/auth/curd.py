...
from app.domains.auth.schema import UserCreate, UserSelect, UserUpdate
from app.domains.base_curd import Curd
from app.models.auth import Permission, Role, RolePermissionLink, User, UserRoleLink
...
class UserCurd(Curd):
    ...
    @classmethod
    async def create(
        cls, session: AsyncSession, user_in: UserCreate, update: dict | None = None
    ) -> User:
        update = update or {}
        update.update(
            {
    ...
    @classmethod
    async def update(
        cls, session: AsyncSession, user_in: UserUpdate, update: dict | None = None
    ) -> User:
        update = update or {}
        if user_in.password:
            update.update(
