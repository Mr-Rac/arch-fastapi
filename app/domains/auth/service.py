...
from app.domains.auth.curd import PermissionCurd, RoleCurd, UserCurd
...
class AuthService(BaseService):
    ...
    async def login(
        ...
        access_token = await Token.create(
            redis, TokenType.ACCESS, user.username, user.scopes
        )
        refresh_token = await Token.create(
            redis, TokenType.REFRESH, user.username, user.scopes
        )
    ...
    async def logout(
        ...
        user = await UserCurd.select(session, UserSelect(username=body.username))
        await Token.clear(redis, user.username)

    @classmethod
    async def select_user(
        cls, session: AsyncSession, user_in: UserSelect
    ) -> UserPublic:
    ...
    async def select_role(
        cls, session: AsyncSession, role_in: RoleSelect
    ) -> RolePublic:
    ...
    async def select_permission(
        cls, session: AsyncSession, permission_in: PermissionSelect
    ) -> PermissionPublic:
    ...
    async def update_permission(
        session: AsyncSession, permission_in: PermissionUpdate
    ) -> PermissionPublic:
